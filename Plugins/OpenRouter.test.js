...
describe('OpenRouter Plugin', () => {
  ...
  describe('completion', () => {
    ...
    it('throws error if config missing', async () => {
      ...
      await expect(OpenRouter.completion([])).rejects.toThrow('OpenRouter configuration file not found');
    });
    ...
    it('handles general API errors', async () => {
      ...
      await expect(OpenRouter.completion([])).rejects.toThrow('API Request failed: 500 Server Error - Internal Error');
    });
  });
});
