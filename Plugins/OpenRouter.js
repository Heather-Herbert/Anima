...
const loadProviderConfig = () => {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error(
      `OpenRouter configuration file not found at ${configPath}. Please create it in the settings folder with endpoint, apiKey, and model.`,
    );
  }
...
const completion = async (messages, tools = null) => {
  ...
  if (response.status === 401 || response.status === 403) {
    throw new Error(
      `Invalid or expired API Key (HTTP ${response.status}). Please check settings/openrouter.json.`,
    );
  }
